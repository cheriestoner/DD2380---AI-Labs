...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        # It is recommended you use the np.random module
        action = None
        epsilon_delta = epsilon_final - epsilon_initial
        epsilon_current = epsilon_initial + epsilon_delta * (current_total_steps / anneal_timesteps)
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        N = 10   # keep track of the highest R per episode for the last N episodes
        R_last_N = np.zeros(N)
        not_reach_highest_reward = True
        ...
        while episode <= self.episode_max and diff > self.threshold or not_reach_highest_reward:
        ...
        # ADD YOUR CODE SNIPPET BETWEENEX. 2.3
            ...
            while not end_episode:
                ...
                action = None
                # action = random.choice(list_pos)   # random action int
                # action = np.nanargmax(Q[s_current])
                action = epsilon_greedy(Q, s_current, list_pos, current_total_steps=current_total_steps, epsilon_final=self.epsilon_final)
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
                ...
                Q_new = (1.0 - lr) * Q[s_current, action] + lr * (R + discount * np.nanmax(Q[s_next, :]))
                Q[s_current, action] = Q_new
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
                # print("current state: {}, next state: {}".format(s_current, s_next))
                s_current = s_next
                current_total_steps += 1
                steps += 1
                # print("Q_{}: {}, action: {}, reward: {}".format(s_current, Q[s_current], action_str, R))
            R_last_N[episode % N] = R_total
            if np.max(R_last_N) == R_total: not_reach_highest_reward = False
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            # Compute the absolute value of the mean between the Q and Q-old
            # diff = np.nanmean(np.abs(Q_old - Q))
            diff = np.nanmax(np.abs(Q_old - Q))
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
            print(
    ...
    def random_agent(self):
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                action = None   # int index of action
