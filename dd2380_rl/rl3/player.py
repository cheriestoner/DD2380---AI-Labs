    ...
    def q_learning(self):
        ...
        # ADD YOUR CODE SNIPPET BETWEENEX. 2.3
            ...
            while not end_episode:
                ...
                action = epsilon_greedy(Q, s_current, list_pos, current_total_steps=current_total_steps)
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
            ...
            diff = 100
            def abs_max_diff(P, Q):
                return np.nanmax(np.abs(P - Q))

            def dist_diff(P, Q, row, col):
                mean = 0.0
                for i in range(row):
                    sum_row = 0.0
                    for j in range(col):
                        if np.isnan(P[i][j]) or np.isnan(Q[i][j]): continue
                        sum_row += (P[i][j] - Q[i][j]) ** 2
                    mean += sum_row ** 0.5
                return mean / row
                
            diff = abs_max_diff(Q_old, Q)
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
            print(
